...
void HttpAcceptor::acceptConnection() {
    ...
    acceptor_.async_accept(
        *socket, [this, socket](const boost::system::error_code &ec)
        {
            if (!ec) Router::RouteRequest(socket);
            
            else  std::cerr << "ERROR: " << ec.value() << " MESSAGE: " << ec.message() << std::endl;


            if (!is_stopped_.load()) acceptConnection();    // continue accepting connections
        }
    );
}
...
void HttpServer::start(unsigned short port, unsigned int thread_pool_size) {
    ...
    try {
        ...
        for (size_t i = 0; i < thread_pool_size; ++i) {
            std::unique_ptr<std::thread> new_thread(new std::thread([this]() { ioc_.run(); } ));
